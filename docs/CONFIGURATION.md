...
| `revalidate_batch` | `MXLRC_TIMING_VALIDATION_REVALIDATE_BATCH` | `100` | How many sidecars one cycle judges. Modest on purpose: each candidate costs a read of the `.lrc`, and on a cold duration cache one bounded header read of its companion audio to bank that duration - paid once per file version, never once per cycle, so a small batch keeps a parked library array quiet. Values below `1` reset to the default. |
