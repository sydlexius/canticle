...
| `innertube_cooldown_seconds` | `0` | Minimum seconds between InnerTube requests. `0` falls back to `api.cooldown`. A negative value is not a disable request and also falls back. The InnerTube client enforces its own floor (2s) on any positive value, so this key can raise the interval but never lower it past that floor. Precedence is `innertube_cooldown_seconds` > `api.cooldown`, same pattern as `petitlyrics_cooldown_seconds`. One InnerTube lookup costs three outbound requests (search, next, browse) versus one for the other two providers, so the same cooldown value buys roughly a third of the lookup rate on this lane. |
