...
- **Recent outcomes.** The 20 most recently completed tracks (artist, title, album, result class, the provider lane that served it, and the completion time in the server's timezone when `TZ` is set, otherwise UTC). A synced result carries a colored tier badge: word-synced (green) or line-synced (blue) reads as such in the badge text; a tier-unknown synced result gets a muted grey badge but the badge text itself still just reads "synced" - the color, not the label, is what marks it as tier-unknown.
...
- **ambiguous** - zero or multiple candidates on either side, or a pairing too
  close to call. Both name tiers can report a near-tie: `heuristic` when the
  pair fails to beat the orphan's best rival in the directory by `min_margin`,
  `heuristic-nm` when the best score fails to beat its runner-up by the same.
...
**Keeping it current.** The command re-examines the tracks that are settled when you run it. The [`[word_sync_recheck]`](CONFIGURATION.md#word_sync_recheck) section (off by default) is the serve-mode sweep that does this continuously, with `batch` capping how many tracks wait at once. Enable it instead of re-running the command to pick up newly settled tracks. The same per-track cost applies, and the tracks it queues count as deferred while they wait.
...
- Result class: `word-synced` or `line-synced` (an `.lrc` was written; see below), `synced` (an `.lrc` was written but its word/line tier was never recorded), `unsynced` (a `.txt` of plain unsynced lyrics), `instrumental` (a `.txt` instrumental marker, from either audio detection or a provider flag), `rejected` (the language/script guard refused the fetched lyric; nothing was written), `miss` (the track exhausted its miss budget with no lyrics found), or `unknown` (a row that predates result classification and cannot be reclassified from anything on disk).

**Word-synced vs. line-synced (#627).** Both tiers write an `.lrc`, but they are not the same result:
- **word-synced** means word-level timing landed on disk - the terminal tier. Nothing further to gain from a re-fetch or a word-sync recheck.
- **line-synced** means only line-level timing exists; every word-capable source that was asked found no word data. Still eligible for a later word-sync recheck to promote it.
- **synced** (rendered as plain "synced" text with a muted grey tier badge, no wording difference from an ordinary "synced" - see the Dashboard section above) means the tier was never recorded, or was recorded but the row was later remediated by the timing guard (quarantined or demoted; such a row's stale tier is deliberately not shown). The tier-unknown bucket is NOT mostly legacy rows: on a default install it includes every cache hit, every result from a lane with no word data (e.g. innertube), every fetch under `word_sync_mode = off`, and every completion where no word-capable lane answered - all of which happen on the current build, every day. A row currently mid word-sync recheck (`word_timing_state = 'queued'`) is flipped out of `done` for the duration of the recheck, so in practice it almost never shows up in this table at all; it does still count toward the tier-unknown tile below. It is shown honestly as tier-unknown rather than guessed from the file contents. [`canticle scan reconcile-word-sync`](CLI_REFERENCE.md#reconcile-word-sync) or the `[word_sync_recheck]` sweep is what resolves most of this bucket over time; issue #1075 (classifying existing `.lrc` files by sync tier) will resolve most of the rest without a recheck.

**Source of truth.** The tier is read from the recorded `word_timing_state` database column (`served` = word-synced, `absent` = line-synced, anything else = tier unknown), not by re-reading the `.lrc` file to detect word-level tags. A recorded column costs one query against `work_queue` (a full scan, not an indexed one - `EXPLAIN QUERY PLAN` shows `SCAN work_queue`, since the tile count deliberately includes both `done` and mid-recheck rows and no single index covers that combination) that a dashboard can run on every page load; that is still cheap in absolute terms (a few milliseconds at a library's typical size) and categorically cheaper than re-parsing every completed track's sidecar on every render, and the existing `outcome_type` column already follows the same recorded-at-completion pattern. A legacy row's tier is not backfilled by inspection - it is shown as tier-unknown rather than guessed.
...
**Lane-attribution caveat (from v1.7.0).** On a deployment whose `[providers].primary` was not `musixmatch`, rows this table recorded during that window name the wrong lane, from the same defect described under the [metrics endpoint](#metrics-endpoint): the primary lane's name came from a hardcoded constant, so the primary provider's work was recorded against `musixmatch`. Unlike the `/metrics` counters, these rows do carry per-track identity and are therefore correctable in principle, but no correction has been applied.

How far you can read through the misnaming depends on the configuration. Where the primary was the only lane, or where the fallbacks were all other providers, it is a uniform relabel: the per-track hit and miss structure is intact and only the name on it is wrong, so a reader who knows which provider was primary can read through it, and this report stays the better instrument for the period. Where `musixmatch` was *also* configured in `fallback_order` behind a different primary, it is not: the primary lane's hardcoded name and the real Musixmatch fallback collide on one label, and `lane_attempts` is `UNIQUE(queue_id, lane)`, so the two lanes' attempts upsert over each other rather than being recorded separately. A track's rows can also mark that shared label as a hit when the primary won, because a lane's hit is decided by comparing the winning lane's NAME. In that configuration the per-lane split for the window is not recoverable from this table either, and neither instrument answers a per-lane question about it.
