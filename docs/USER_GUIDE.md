...
**Keeping it current.** The command re-examines the tracks that are settled when you run it. The [`[word_sync_recheck]`](CONFIGURATION.md#word_sync_recheck) section (off by default) is the serve-mode sweep that will do this continuously, with `batch` capping how many tracks wait at once. It is not used yet; until the sweep ships, re-run the command to pick up newly settled tracks. The same per-track cost applies.
