...
- [LRC Normalization](lrc-normalization.md) - why stacked-timestamp LRC renders incorrectly in simple players, the detect (startup check) vs. apply (`scan reconcile-lrc --yes`) split, and the `.lrc.orig` backup.
