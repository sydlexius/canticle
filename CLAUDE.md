...
- `realign` -- confidence resolver (`Realigner`, `Move`, `Apply`) that re-attaches orphaned `.lrc`/`.txt` sidecars to renamed or moved audio: exact ISRC/MBID provenance, filesystem heuristic with a Jaro-Winkler name guard, an opt-in N:M name-similarity matcher (`heuristic-nm`, config `name_match`/`min_margin`) for a directory with multiple orphans and multiple sidecar-less audio files, ambiguous, conflict. Backup-first and clobber-safe; shared by the `realign` CLI command and serve-mode reactive realign (watcher / scan / Lidarr-webhook).
