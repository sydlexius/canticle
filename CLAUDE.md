...
- `orchestrator` -- multi-lane orchestration (`Lane`, `Orchestrator`, parallel-race + suitability scoring); composes `providers` with per-lane `circuit` breakers. A result ends the dispatch only if it is suitable AND `lyrics.DecidePromotion` would promote it as-is, judged against the query track's `TrackLength` (the audio duration the writer later judges against) (#950): a categorical (quarantined) result falls through to the remaining lanes and ranks below any other result, and, when it is the only result, settles exactly as before (done + `timing_outcome=categorical`, nothing written, never cached) even if another lane was breaker-open/throttled/not ready -- waiting on such a lane is left to a follow-up; a cached refused entry reads as a miss; a suitable MisSynced/degenerate one is HELD (first lane wins ties, outranks every non-suitable result, and skips the instrumental-only detector lane). No threshold lives here.
