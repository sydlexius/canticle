...
- `innertube` -- YouTube Music's unauthenticated internal ("innertube") API adapter, tokenless like petitlyrics; owns the three-call flow (search, next, browse) and the client-string selection that picks a timed-cue response over a plain-text one (`ANDROID_MUSIC`/`IOS_MUSIC`, never `WEB_REMIX`), its own pacer with a 2s policy floor (`MinAllowedInterval`, distinct from petitlyrics' 10s), and search-result verification (the search call never signals "no match", so a caller must independently confirm a candidate corresponds to the requested track). This lane MULTIPLEXES per track between upstream lyric licensors (Musixmatch or LyricFind) rather than being its own source; it reports the per-result licensor separately from the lane name so `[source:innertube]` stays constant while `[upstream:]` carries the licensor -- see `docs/provider-attribution.md` and `docs/provider-terms.md`.
...
- `orchestrator` -- multi-lane orchestration (`Lane`, `Orchestrator`, parallel-race + suitability scoring); composes `providers` with per-lane `circuit` breakers. A result ends the dispatch only if it is suitable AND `lyrics.DecidePromotion` would promote it as-is, judged against the query track's `TrackLength` (the audio duration the writer later judges against) (#950): a categorical (quarantined) result falls through to the remaining lanes and ranks below any other result, and, when it is the only result, is returned with a nil error once every lane answered (a transport failure, incl. a request-shape 403, counts as an answer) so the row settles as before (done + `timing_outcome=categorical`, nothing written, never cached); while a lane did NOT answer (breaker open, throttled/auth-failing, not ready) it is returned WITH `ErrTimingRefusedUntried` for the worker's bounded wait; a cached refused entry reads as a miss; a suitable MisSynced/degenerate one is HELD (first lane wins ties, outranks every non-suitable result, and skips the instrumental-only detector lane). No threshold lives here.
...
- `lyrics` -- LRC/TXT/instrumental writer (`Writer`, `LRCWriter`), `Slugify`, an `.lrc` parser, provenance-tag embedding, and fsync helpers. Also owns the accept-time timing guard (`DecidePromotion`, #439): `WriteLRC` consults `timing.Evaluate` before promoting a synced result, refusing the `.lrc` and keeping the words as `.txt` on `MisSynced` (an already-settled sidecar is left untouched -- that is how the writer distinguishes an upgrade from a fresh fetch, by statting the target rather than taking a caller flag) and writing nothing on `Categorical`. Unknown duration always fails open. The duration judged against is `Song.AudioDurationSeconds`, which callers stamp from the AUDIO FILE, never `Track.TrackLength` (the provider catalog value the lyric was already timed against).
  `ReadSyncedLRC`/`EvaluateLRCFile` are the disk-to-`models.Song` seam (#442): they read a sidecar back off disk through `lrcnormalize.ParseBody` and hand it to `timing.Evaluate`, so an existing file is re-judged by the SAME predicate that judged it at accept time. `PlainBody` flattens cues to the words a demotion persists, sharing `timing.IsDecorative` with the guard.
...
- `cache` -- lyrics cache repository (`CacheRepo`) over SQLite. `Lookup` falls back to the bucket-0 unknown-duration sentinel row on an exact-bucket miss, so `Invalidate` (used by `purgeprovenance`) deletes every duration bucket for a key; it also takes an `Execer`, so a caller can invalidate inside its own transaction.
...
- `audiodur` -- exact per-file audio duration cache (`Store`) keyed by path and validated by (mtime, size); filled opportunistically wherever a file is already open, from that read's own handle rather than a path re-stat. A miss is `unknown_duration`, never an error (#441), and `revalidate` (#442) is its consumer: a miss fails open there rather than remediating. The scanner ALSO fills it for files it skips for fetching (#684): a settled sidecar short-circuits the scan ~200 lines before the enrichment probe, so files that already had a sidecar were never duration-probed by an older build -- a coverage gap by construction rather than a lookup or invalidation failure, which is what the measurement showed for the bulk of the population (a full scan banked 690 durations and moved `unknown-duration` by zero). That is the LIKELIEST cause of a large unknown-duration count, not the only one: `revalidate.durationOf` also reads unknown when no duration store is wired and when a companion audio file cannot be stat'ed, neither of which a re-scan fixes. That fill is gated on a `Lookup`, so a skipped file costs one header read per file VERSION, not one per scan; ungated it would keep the library disks permanently awake, which is the symptom #684 exists to remove.
- `revalidate` -- re-judges `.lrc` files ALREADY on disk against their companion audio's exact duration and plans remediation (#442): demote a `MisSynced` lyric's words to `.txt`, quarantine a `Categorical` one. Owns no predicate and no filesystem machinery -- the verdict is `timing.Evaluate` via the `lyrics` disk-to-Song seam, the duration is `audiodur`'s, and every mutation goes through `realign.Apply`. `Plan` and `Apply` are separate calls, so the CLI's dry-run default provably writes nothing; unknown duration never remediates; removal is a MOVE (quarantine) unless `--purge`. Two entry points share one core: `Plan` WALKS library roots (the CLI), and `PlanCandidates` judges an explicit list of `work_queue` rows without reading a single directory (the serve-mode sweep, #443) -- a row records its audio path, so the sidecar is DERIVED (`stem + .lrc`) rather than discovered, and a cycle costs its batch, never the size of the library. In candidate mode EVERY candidate returns a finding, including the un-judgeable ones (`no_sidecar`/`no_audio`/unknown duration), because the caller stamps what it gets a finding for and an unstamped row head-of-lines the oldest-first batch forever. Its two failures are separated by BLAST RADIUS: an unparsable `.lrc` concerns one file, so it is counted and the row is left UNSTAMPED (an unreadable file is equally what a transient I/O error or an unavailable mount looks like, and the stamp is one-way, so it is retried rather than retired); a duration-store failure concerns the whole batch, so it abandons the cycle rather than stamping rows it never judged.
...
- `worker` -- durable-queue `Worker` that drains work items through the providers/orchestrator and cache. On `ErrTimingRefusedUntried` (#950) it parks ONLY that row via `queue.DeferRefused` (its own `refused_waits` counter; attempts/miss_count untouched; wait = the breaker cap) and keeps draining, RESETTING `consecutiveFailures` to 0; the detector's miss telemetry is stamped before the park. An instrumental-only (detector) lane never counts as untried. After `maxRefusedWaits` (3) it settles the carried refused song as done + categorical, logged at Warn with the untried lane; the wait is bounded but does not guarantee the lane is ever probed (another row can re-trip its breaker), so retrying such rows is a #950 follow-up.
...
- `watcher` -- optional filesystem watcher that triggers targeted library scans on change; complements, never replaces, the periodic scheduler. Consults `selfwrite` in `translate` to drop the events canticle's own sidecar writes generate, before they can prune or arm a debounce timer (#685).
- `selfwrite` -- a TTL'd, concurrency-safe set of paths this process just wrote, shared in memory between the `lyrics` writer (which records) and the `watcher` (which suppresses). A leaf package, so neither side depends on the other. Suppression keys on the FILE path, never its directory, so an external change to a sibling in the same album folder still triggers a rescan; a recorded final path also covers its `os.CreateTemp` temp file by derivation, closing the race where the temp file's Create event outruns the writer's knowledge of the random name. Entries expire (serve wires the TTL to a small multiple of `watcher.debounce_ms`), so a crash or an undelivered event can never leave a path permanently deaf to external change. A nil `*Registry` is a safe no-op for every non-serve caller.
...
- `timing` -- the shared pure predicate classifying synced-lyric timing against audio duration; sole owner of `TimingOutcome`/`Evaluate`, the exported `IsDecorative` line classifier, and the calibrated `Tolerance`/`CategoricalRatio` constants (#438). Consumed by all four callers now: the accept-time guard in `lyrics`, the worker's outcome stamp, the `revalidate` backlog pass (#442), and the serve-mode sweep (#443). **The thresholds are NOT configurable and that omission is the design**: `Tolerance` and `CategoricalRatio` are a co-calibrated pair (tolerance is checked FIRST, so on short audio a large ratio still reads `Ok`), and one predicate serving all four callers is what keeps a `timing_outcome` comparable across a deployment's own history. #443's original AC listed a `categorical_ratio` config key; it was deliberately not built. A threshold worth tuning belongs here, where every caller moves together. No I/O; the max timestamp is taken over text-bearing lines only, decorative lines filtered wherever they appear, so a decorative marker past the audio end is not an overrun.
- `config` -- TOML config resolution (XDG paths, registry-driven keys, token precedence CLI > env > file) plus redaction, validation, render/write. Owns the `[timing_validation]` surface (#443), which is now LIVE: its consumer is `commands.runTimingValidationSweep`. `enabled` / `revalidate_existing` gate the serve-mode sweep (BOTH required, mirroring the realign gate), `revalidate_batch` budgets one cycle, and `on_mis_synced` / `on_categorical` select the remediation. The two action enums differ by exactly one value and the difference is load-bearing -- a categorical lyric is another song's words, so `demote` is illegal there; both sets derive from one pair of functions that `enumValues`, the settings dropdown, and the file/env re-default checks all read, so they cannot drift. An unrecognized action ALWAYS resets to the conservative default rather than falling through, so a typo can never resolve to something more destructive than what was written.
- `logging` -- `slog` logger setup and secret redaction.
- `realign` -- confidence resolver (`Realigner`, `Move`, `Apply`) that re-attaches orphaned `.lrc`/`.txt` sidecars to renamed or moved audio: exact ISRC/MBID provenance, filesystem heuristic with a title-only Jaro-Winkler name guard plus a `min_margin` runner-up rule against the directory's other audio (#672: the artist is excluded and both name tiers must agree on a pair, or a shared artist name renames a sidecar onto the wrong song), an opt-in N:M name-similarity matcher (`heuristic-nm`, config `name_match`/`min_margin`) for a directory with multiple orphans and multiple sidecar-less audio files, ambiguous, conflict. Backup-first and clobber-safe; shared by the `realign` CLI command and serve-mode reactive realign (watcher / scan / Lidarr-webhook). `Apply` also carries the REMEDIATION action kinds (`KindDemote`/`KindQuarantine`/`KindPurge`, #442) so `revalidate` reuses this one apply path and one JSONL backup trail rather than standing up a second remediation stack; `KindRename` is the zero value, so every pre-existing `Move` is unchanged.
- `commands` -- the CLI command tree: top-level `Args` and every subcommand (`fetch`, `serve`, `scan`, `library`, `keys`, `secrets`, `config`, `queue`, `provenance`, `realign`, `revalidate`, `completion`). The `revalidate` subcommand is thin CLI wiring over the `revalidate` package, dry-run by default behind `--apply` (not `--yes`) and AGGREGATE-ONLY on stdout -- no path, artist, title, or lyric text is ever printed, since a sidecar path carries the library's private metadata; `--tail` is the one place per-file detail lands. `runTimingValidationSweep` (#443) is its serve-mode counterpart: the same core, driven from the `queue.ListTimingBacklog` watermark instead of a walk, gated on `timing_validation.enabled` AND `.revalidate_existing`, applying BEFORE it stamps so a failed remediation leaves its row in the backlog to retry. The `realign` subcommand is thin CLI wiring over the `realign` package (above); `scan reconcile-paths` drives `prune` on demand and `scan reconcile-identity` drives `identityrepair` on demand. All are dry-run by default and write a JSONL backup of what they applied, following `scan reconcile`; the confirmation flag is `--yes` for the reconcile family and `--apply` for `revalidate`.
