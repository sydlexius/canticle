...
func TestWordRecheck_ServedWritesWordResult(t *testing.T) {
	...
	if row.missCount != 0 || row.attempts != 0 || row.laneRows != 1 {
		t.Fatalf("row counters = %+v; want miss 0, attempts 0, one lane_attempts row", row)
	}
...
func TestWordRecheck_NoWordsNeverDowngrades(t *testing.T) {
	cases := []struct {
		...
		{"line-synced, no words", &fakeFetcher{song: recheckSong("line only", false, models.WordAnswerAbsent)}},
		...
		{"instrumental result", &fakeFetcher{song: models.Song{Track: models.Track{Instrumental: 1}, WordAnswer: models.WordAnswerAbsent}}},
		{"no match on every lane", &fakeFetcher{err: musixmatch.ErrNotFound}},
...
func TestWordRecheck_UnansweredStaysQueued(t *testing.T) {
	cases := []struct {
		...
		{"unknown word answer", &fakeFetcher{song: recheckSong("line only", false, models.WordAnswerUnknown)}, nil},
		{"throttled lane", &fakeFetcher{err: musixmatch.ErrRateLimited}, errThrottled},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			rig, w := newRecheckRig(t, tc.primary, &fakeFetcher{err: petitlyrics.ErrNoMatch}, false)
...
// TestWordRecheck_CacheAcceptsOnlyWordSynced: a line-synced cache entry reads
// as a miss and the lane is asked; a word-synced one is written with zero
// lane calls.
func TestWordRecheck_CacheAcceptsOnlyWordSynced(t *testing.T) {
	for _, words := range []bool{false, true} {
		primary := &fakeFetcher{err: musixmatch.ErrNotFound}
		rig, w := newRecheckRig(t, primary, &fakeFetcher{err: petitlyrics.ErrNoMatch}, false)
		entry, err := encodeSong(recheckSong("cached line", words, models.WordAnswerUnknown))
		if err != nil {
			t.Fatal(err)
		}
		...
		wantCalls, wantState := 1, queue.WordTimingAbsent
		if words {
			wantCalls, wantState = 0, queue.WordTimingServed
		}
		if primary.calls != wantCalls || rig.recheckRow(t).state != wantState {
			t.Fatalf("words=%v: lane calls %d, state %q; want %d, %q", words, primary.calls, rig.recheckRow(t).state, wantCalls, wantState)
		}
	}
}
