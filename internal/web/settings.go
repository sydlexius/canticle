...
func rawConfigValue(cfg config.Config, path string) string {
	...
	case "instrumental_detector.backfill.enabled":
		return strconv.FormatBool(cfg.InstrumentalDetector.Backfill.Enabled)
	case "instrumental_detector.backfill.batch_size":
		return strconv.Itoa(cfg.InstrumentalDetector.Backfill.BatchSize)
	case "instrumental_detector.backfill.interval_minutes":
...
var settingsLabels = map[string]string{
	...
	"instrumental_detector.backfill.enabled":          "Check older tracks for instrumentals in the background",
