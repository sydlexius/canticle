...
// injectEditorTag wraps lyrics.InjectEditorTag: ErrChangedDuringRewrite (a
// concurrent writer won this file, #483 finding 3) reads as skipped, not
// errored -- never stamped done, retried next pass.
func injectEditorTag(path string) (bool, error) {
	stamped, err := lyrics.InjectEditorTag(path)
...
// editorTagBackupRecord is one JSONL line per stamped file.
type editorTagBackupRecord struct {
	FilePath string `json:"file_path"`
}

func appendEditorTagBackup(f *os.File, path string) error {
	b, err := json.Marshal(editorTagBackupRecord{FilePath: path})
	if err != nil {
...
func walkEditorTagRoot(ctx context.Context, root string, fn func(path string) (stamped bool, err error)) (scanned, stamped, errored, degraded int, err error) {
	_, canonRoot := pathutil.CanonicalRoot(root)
	if _, statErr := os.Stat(canonRoot); statErr != nil {
		return 0, 0, 0, 0, fmt.Errorf("walk %s: %w", root, statErr)
	}
	walkErr := filepath.WalkDir(canonRoot, func(path string, d fs.DirEntry, walkFileErr error) error {
		...
		k := sidecar.KindOf(d.Name())
		if k != sidecar.KindLineSynced && k != sidecar.KindWordSynced {
			return nil
		}
		scanned++
		ok, ferr := fn(path)
	...
	if walkErr != nil {
		return scanned, stamped, errored, degraded, fmt.Errorf("walk %s: %w", root, walkErr)
	}
	return scanned, stamped, errored, degraded, nil
}
...
func runReconcileEditorTag(ctx context.Context, out io.Writer, args ScanReconcileEditorTagCmd) int {
	...
	perFile := func(path string) (bool, error) {
		if !args.Yes {
			return lyrics.EditorTagEligible(path)
		}
		eligible, eerr := lyrics.EditorTagEligible(path)
		if eerr != nil || !eligible {
			return false, eerr
		}
		// Write-ahead: the backup record is durable before the file is
		// touched, matching every other reconcile-family command.
		if backupFile == nil {
		...
		if berr := appendEditorTagBackup(backupFile, path); berr != nil {
			return false, berr
		}
		return injectEditorTag(path)
	}

	var scanned, stamped, errored, degraded int
	for _, root := range roots {
		s, st, e, dg, werr := walkEditorTagRoot(ctx, root, perFile)
		scanned += s
		stamped += st
		errored += e
		degraded += dg
	...
	_, _ = fmt.Fprintf(out, "reconcile-editor-tag: scanned %d .lrc/.elrc file(s); %s %d, errored %d, degraded %d%s\n",
		scanned, verb, stamped, errored, degraded, suffixDryRun(args.Yes))
	if backupFile != nil {
...
// `scan reconcile-editor-tag --yes` is the manual backstop). Quiet, no backup.
//
...
// did not cover every root or hit a per-file error -- retried next boot.
// Per-root availability reuses #470's own MediaEntries signal (dry-run walk).
//
// BOUNDED DEGRADED RETRY (#1084): a permanently degraded root (bad
// permissions, a stale/decommissioned library) or a per-file error that never
// clears would otherwise re-walk the whole library on every boot forever --
// the same failure mode #922 fixed for the #470 stacked-.lrc check. This pass
// now keeps its OWN counter (editorTagBackfillDegradedAttemptsMarker) under
// the shared incrementDegradedAttempts/clearDegradedAttempts helpers and the
// same maxDegradedAttempts ceiling: past that many consecutive degraded
// startups, the done-marker is stamped anyway (a give-up, logged once at
// Warn with counts only, never a path) instead of retrying forever. A clean,
...
// #1084 tracks.
func runEditorTagBackfill(ctx context.Context, sqlDB *sql.DB, selfWrites *selfwrite.Registry) {
	...
	for _, l := range libs {
		mediaSummary, merr := runStackedWalk(ctx, lrcbackfill.Options{Roots: []string{l.Path}, Apply: false, Quiet: true})
		if merr != nil {
			if errors.Is(merr, context.Canceled) {
				return
			}
			slog.Warn("editor-tag backfill: root unavailable; will retry on next startup", "library_id", l.ID, "cause", classifyWalkError(merr))
			rootUnavailable = true
			continue
		}
		if mediaSummary.MediaEntries == 0 {
			slog.Warn("editor-tag backfill: root appears empty or not mounted yet; will retry on next startup", "library_id", l.ID)
			rootUnavailable = true
			continue
		}
		s, st, e, dg, werr := walkEditorTagRoot(ctx, l.Path, perFile)
		scanned += s
		stamped += st
		errored += e
		degraded += dg
	...
	if rootUnavailable || degraded > 0 || errored > 0 {
		...
		if attempts < maxDegradedAttempts {
			slog.Warn("editor-tag backfill: walk did not cleanly cover every configured root this startup; will retry next startup",
				"scanned", scanned, "stamped", stamped, "errored", errored, "degraded", degraded,
