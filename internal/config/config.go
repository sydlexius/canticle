...
type OutputConfig struct {
	...
	// silently strip them, or display them as literal text. Only the third is
	// visibly broken, so a library must not gain them merely because a provider
	// served richer data.
	//
...
type ProvidersConfig struct {
	...
	// PetitLyricsCooldownSeconds is the minimum gap between Petit Lyrics
	// requests. It exists because the two providers share neither a rate-limit
	// budget nor a throttle profile, so api.cooldown (which means Musixmatch)
	// was the wrong knob for this lane (#535).
	//
	// Default 0, which means "fall back to api.cooldown" -- the behavior every
	// deployment had before this key existed. A negative value is not a
	// disable request (this lane is never unpaced by configuration) and also
	// falls back.
	//
	// This is the interval REQUESTED. The client clamps any positive value up
	// to petitlyrics.MinAllowedInterval, so the effective floor is a client
...
type QueueConfig struct {
	...
	// BatchSize is the shuffled lookahead buffer size (#571): Dequeue draws this
	// many eligible rows at random once per batch and serves them in a stamped
	// order, moving the RANDOM() shuffle from per-claim to per-batch while keeping
	// the anti-fingerprint property. Defaults to 10. Set queue.batch_size = 0 (or
	// MXLRC_QUEUE_BATCH_SIZE=0) to disable batching and restore the per-item
	// random path -- a clean production rollback. Only meaningful when Randomize
	// is true; ignored in deterministic mode.
	BatchSize int `toml:"batch_size"`
}
...
func applyEnvOverrides(cfg *Config, applied map[string]bool) {
	...
	if v := os.Getenv("MXLRC_WORD_SYNC"); v != "" {
		wordSync, err := strconv.ParseBool(v)
		if err != nil {
			slog.Warn("env var is invalid; using current value", "var", "MXLRC_WORD_SYNC", "value", v, "current", cfg.Output.WordSync) //nolint:gosec // reason: G706: tainted env var passed as a structured slog field value (not a format string); no log-injection vector since slog escapes values
		} else {
	...
	if v := os.Getenv("MXLRC_QUEUE_BATCH_SIZE"); v != "" {
		n, err := strconv.Atoi(v)
		if err != nil {
			slog.Warn("env var is invalid; using current value", "var", "MXLRC_QUEUE_BATCH_SIZE", "value", v, "current", cfg.Queue.BatchSize) //nolint:gosec // G706: tainted env var passed as a structured slog field value (not a format string); no log-injection vector since slog escapes values
		} else {
			cfg.Queue.BatchSize = n
			applied["queue.batch_size"] = true
		}
	}
