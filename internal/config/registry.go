...
var fields = []FieldSpec{
	...
	// [timing_validation]
	{Path: "timing_validation.enabled", Section: "timing_validation", Type: TypeBool, EnvVars: []string{"MXLRC_TIMING_VALIDATION_ENABLED"}, Criticality: Safe, Editable: true, Description: "Master switch for the serve-mode timing sweep, which re-judges .lrc files already on disk against their companion audio's exact duration. Off by default; the revalidate CLI command runs regardless."},
	{Path: "timing_validation.revalidate_existing", Section: "timing_validation", Type: TypeBool, EnvVars: []string{"MXLRC_TIMING_VALIDATION_REVALIDATE_EXISTING"}, Criticality: Caution, Editable: true, Description: "Allow the sweep to drain the backlog of .lrc files written before the accept-time timing guard existed. Off by default. Both this and timing_validation.enabled must be true for the sweep to run."},
	{Path: "timing_validation.revalidate_batch", Section: "timing_validation", Type: TypeInt, EnvVars: []string{"MXLRC_TIMING_VALIDATION_REVALIDATE_BATCH"}, Criticality: Safe, Editable: true, Description: "How many sidecars one sweep cycle judges. Default 100, kept modest so the backlog drains over many cycles rather than in one burst that wakes the library disks. Values below 1 are reset to the default."},
