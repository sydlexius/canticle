...
var fields = []FieldSpec{
	...
	{Path: "timing_validation.revalidate_batch", Section: "timing_validation", Type: TypeInt, EnvVars: []string{"MXLRC_TIMING_VALIDATION_REVALIDATE_BATCH"}, Criticality: Safe, Editable: true, Description: "How many sidecars one sweep cycle judges. Default 100, kept modest so the backlog drains over many cycles rather than in one burst that wakes the library disks. A row that was judged AND acted on is stamped and never re-judged; only unstamped rows (a failed action, or a verdict that could not be reached) are re-read and re-judged on a later cycle. So a small batch costs elapsed time rather than repeating settled work. Values below 1 are reset to the default."},
