...
var fields = []FieldSpec{
	...
	{Path: "timing_validation.revalidate_batch", Section: "timing_validation", Type: TypeInt, EnvVars: []string{"MXLRC_TIMING_VALIDATION_REVALIDATE_BATCH"}, Criticality: Safe, Editable: true, Description: "How many sidecars one sweep cycle judges. Default 100, kept modest so the backlog drains over many cycles rather than in one burst that wakes the library disks. A judged row is stamped and never revisited, so a small batch costs elapsed time, never repeated work. Values below 1 are reset to the default."},
