...
var fields = []FieldSpec{
	...
	{Path: "server.scan_schedule.frequency", Section: "server", Type: TypeString, EnvVars: []string{"MXLRC_SCAN_SCHEDULE"}, Criticality: Caution, Editable: true, Description: "How often to scan the library in serve mode, on the wall clock: hourly, daily, weekly, or off. Leave blank to keep using the deprecated scan_interval_seconds."},
	{Path: "server.scan_schedule.at", Section: "server", Type: TypeString, EnvVars: []string{"MXLRC_SCAN_SCHEDULE_AT"}, Criticality: Caution, Editable: true, Description: "Time of day the daily or weekly scan starts, as HH:MM on a 24-hour local clock (e.g. 04:00)."},
	{Path: "server.scan_schedule.day", Section: "server", Type: TypeString, EnvVars: []string{"MXLRC_SCAN_SCHEDULE_DAY"}, Criticality: Caution, Editable: true, Description: "Day the weekly scan runs. Only used when the frequency is weekly."},
	{Path: "server.scan_schedule.scan_on_start", Section: "server", Type: TypeBool, EnvVars: []string{"MXLRC_SCAN_SCHEDULE_ON_START"}, Criticality: Caution, Editable: true, Description: "Also scan the whole library every time the server starts. Off by default: the filesystem watcher already catches changes, and a restart should not cost a full walk."},
	{Path: "server.sweep_interval_seconds", Section: "server", Type: TypeInt, EnvVars: []string{"MXLRC_SWEEP_INTERVAL"}, Criticality: Caution, Editable: true, Description: "Seconds between path-reconciliation sweeps in serve mode (deletes rows whose source file vanished). Default 21600 (6h); 0 disables the sweep."},
	...
	{Path: "providers.innertube_cooldown_seconds", Section: "providers", Type: TypeInt, EnvVars: []string{"MXLRC_PROVIDERS_INNERTUBE_COOLDOWN_SECONDS"}, Criticality: Safe, Editable: true, Description: "Minimum seconds between InnerTube requests. 0 uses the Musixmatch cooldown; values below the provider's own floor are raised to it. Counted per REQUEST, and a lookup costs one to three of them (three only when this lane serves a result)."},
	...
	{Path: "instrumental_detector.speech_classes", Section: "instrumental_detector", Type: TypeStringSlice, EnvVars: []string{"MXLRC_INSTRUMENTAL_DETECTOR_SPEECH_CLASSES"}, Criticality: Safe, Editable: true, Description: "AudioSet speech labels gated on sustained mean (not peak) so brief incidental speech does not block instrumental (default: Speech)."},
	{Path: "instrumental_detector.speech_max_confidence", Section: "instrumental_detector", Type: TypeFloat64, EnvVars: []string{"MXLRC_INSTRUMENTAL_DETECTOR_SPEECH_MAX_CONFIDENCE"}, Criticality: Safe, Editable: true, Description: "Maximum summed speech-class frame mean before a track is excluded from instrumental (0-1; provisional default 0.20)."},
	...
	{Path: "instrumental_detector.ordering", Section: "instrumental_detector", Type: TypeString, EnvVars: []string{"MXLRC_INSTRUMENTAL_DETECTOR_ORDERING"}, Criticality: Safe, Editable: true, Description: "Detector lane ordering: front (settle instrumental before providers) or demoted (consult only on a provider miss)."},
	{Path: "instrumental_detector.backfill.enabled", Section: "instrumental_detector", Type: TypeBool, EnvVars: []string{"MXLRC_INSTRUMENTAL_DETECTOR_BACKFILL_ENABLED"}, Criticality: Safe, Editable: true, Description: "Serve-mode sweep that classifies never-scored tracks in the background (default on; the scan reconcile-instrumental CLI is unaffected either way)."},
	{Path: "instrumental_detector.backfill.batch_size", Section: "instrumental_detector", Type: TypeInt, EnvVars: []string{"MXLRC_INSTRUMENTAL_DETECTOR_BACKFILL_BATCH_SIZE"}, Criticality: Safe, Editable: true, Description: "Tracks classified per backfill cycle (default 100). Bounds how much one cycle does; cooldown_seconds governs how long it takes."},
	...
	{Path: "word_sync_generate.model", Section: "word_sync_generate", Type: TypeString, EnvVars: []string{"MXLRC_WORD_SYNC_GENERATE_MODEL"}, Criticality: Safe, Editable: true, Description: "Reserved, not yet sent: the aligner client's wire contract has no model parameter, and the sidecar picks its own models via its own ALIGNER_* environment variables."},
	...
	// [realign]
	{Path: "realign.enabled", Section: "realign", Type: TypeBool, EnvVars: []string{"MXLRC_REALIGN_ENABLED"}, Criticality: Safe, Editable: true, Description: "Master switch for the realign feature that re-attaches orphaned .lrc/.txt sidecars to renamed audio files. Not yet active: reserved for the serve-mode auto-realign integration (#450); the realign CLI command runs regardless. Off by default."},
	{Path: "realign.on_scan", Section: "realign", Type: TypeBool, EnvVars: []string{"MXLRC_REALIGN_ON_SCAN"}, Criticality: Caution, Editable: true, Description: "Run realign automatically after each library scan. Not yet active: reserved for the serve-mode auto-realign integration (#450); the realign CLI command runs regardless. Off by default."},
	{Path: "realign.require_provenance", Section: "realign", Type: TypeBool, EnvVars: []string{"MXLRC_REALIGN_REQUIRE_PROVENANCE"}, Criticality: Safe, Editable: true, Description: "Only apply moves backed by a provenance (ISRC/MBID) match; heuristic-tier candidates are reported but never renamed. Off by default."},
	{Path: "realign.cross_directory", Section: "realign", Type: TypeBool, EnvVars: []string{"MXLRC_REALIGN_CROSS_DIRECTORY"}, Criticality: Caution, Editable: true, Description: "Allow exact provenance matches to move a sidecar to an audio file in a different directory within the same library root. Off by default (matches are confined to the orphan's directory)."},
	{Path: "realign.identity_keys", Section: "realign", Type: TypeStringSlice, EnvVars: []string{"MXLRC_REALIGN_IDENTITY_KEYS"}, Criticality: Safe, Editable: true, Description: "Ordered provenance identifiers the exact tier matches on, most authoritative first. Valid values: mbid, isrc. Default: mbid, isrc."},
	{Path: "realign.min_confidence", Section: "realign", Type: TypeFloat64, EnvVars: []string{"MXLRC_REALIGN_MIN_CONFIDENCE"}, Criticality: Safe, Editable: true, Description: "Jaro-Winkler name-similarity floor (0-1) for a heuristic-tier rename. Each side is scored on its most discriminating field -- title, else filename stem; the ARTIST IS EXCLUDED whenever either is available, because every candidate in an album directory shares it. A pair below this floor is reported and skipped. Default 0.75."},
	{Path: "realign.auto_apply_heuristic", Section: "realign", Type: TypeBool, EnvVars: []string{"MXLRC_REALIGN_AUTO_APPLY_HEURISTIC"}, Criticality: Caution, Editable: true, Description: "Whether serve-mode reactive realign (watcher/webhook/post-scan) may auto-apply heuristic (name-similarity) matches. Off by default: unattended realign applies only exact provenance (ISRC/MBID) matches. The manual realign CLI is unaffected."},
	{Path: "realign.name_match", Section: "realign", Type: TypeBool, EnvVars: []string{"MXLRC_REALIGN_NAME_MATCH"}, Criticality: Caution, Editable: true, Description: "Enable the N:M name-similarity matcher for directories with multiple orphaned sidecars and multiple sidecar-less audio files: each orphan is scored against every remaining candidate and paired only when unambiguous. Off by default; such directories are reported ambiguous and skipped."},
	{Path: "realign.min_margin", Section: "realign", Type: TypeFloat64, EnvVars: []string{"MXLRC_REALIGN_MIN_MARGIN"}, Criticality: Safe, Editable: true, Description: "Minimum score separation (0-1) an orphan's best-scoring N:M candidate must hold over its runner-up before the pairing is accepted; a near-tie is reported ambiguous rather than guessed. Default 0.05."},

	// [timing_validation]
	{Path: "timing_validation.enabled", Section: "timing_validation", Type: TypeBool, EnvVars: []string{"MXLRC_TIMING_VALIDATION_ENABLED"}, Criticality: Safe, Editable: true, Description: "Master switch for the serve-mode timing sweep, which re-judges .lrc files already on disk against their companion audio's exact duration and remediates the ones whose cues overrun it. Both this and timing_validation.revalidate_existing must be true for the sweep to run. The revalidate CLI command runs regardless of this flag. Off by default."},
	{Path: "timing_validation.revalidate_existing", Section: "timing_validation", Type: TypeBool, EnvVars: []string{"MXLRC_TIMING_VALIDATION_REVALIDATE_EXISTING"}, Criticality: Caution, Editable: true, Description: "Allow the unattended sweep to touch the backlog of .lrc files written before the accept-time timing guard existed. Both this and timing_validation.enabled must be true for the sweep to run: the master switch says the feature is live, this says a background pass may move or delete pre-existing files. Off by default."},
	{Path: "timing_validation.revalidate_batch", Section: "timing_validation", Type: TypeInt, EnvVars: []string{"MXLRC_TIMING_VALIDATION_REVALIDATE_BATCH"}, Criticality: Safe, Editable: true, Description: "How many sidecars one sweep cycle judges. Default 100, kept modest so the backlog drains over many cycles rather than in one burst that wakes the library disks. A row is stamped once it reaches a terminal verdict with no failed remediation -- including when the action is 'off', where the verdict is recorded and no file is touched -- and a stamped row is never re-judged. Only unstamped rows are re-read on a later cycle: a failed action, and an unreadable or unparsable .lrc (which is left retriable rather than retired, since a transient read failure is not a fact about the file). So a small batch costs elapsed time rather than repeating settled work. Values below 1 are reset to the default."},
	{Path: "timing_validation.on_mis_synced", Section: "timing_validation", Type: TypeString, EnvVars: []string{"MXLRC_TIMING_VALIDATION_ON_MIS_SYNCED"}, Criticality: Caution, Editable: true, Description: "What the sweep does to a lyric that overran the audio but stayed under the categorical ratio. One of: demote (keep the words as .txt, the default, since the words are content-correct), quarantine, purge (irreversible), off (evaluate and record only)."},
	{Path: "timing_validation.on_categorical", Section: "timing_validation", Type: TypeString, EnvVars: []string{"MXLRC_TIMING_VALIDATION_ON_CATEGORICAL"}, Criticality: Caution, Editable: true, Description: "What the sweep does to a lyric whose last cue sits far past the audio end -- the wrong song's words entirely. One of: quarantine (the default), purge (irreversible), off (evaluate and record only). There is no demote here: nothing is worth keeping as .txt."},

	// [word_sync_recheck] -- consumed by the serve sweep (commands.runWordRecheckSweepLoop, #1048).
	{Path: "word_sync_recheck.enabled", Section: "word_sync_recheck", Type: TypeBool, EnvVars: []string{"MXLRC_WORD_SYNC_RECHECK_ENABLED"}, Criticality: Safe, Editable: true, Description: "Master switch for the serve-mode word-timing re-check sweep, which keeps queuing newly settled line-synced tracks for a paced word-timing re-check without an operator running scan reconcile-word-sync. Does nothing while output.word_sync_mode is off. Off by default."},
	{Path: "word_sync_recheck.batch", Section: "word_sync_recheck", Type: TypeInt, EnvVars: []string{"MXLRC_WORD_SYNC_RECHECK_BATCH"}, Criticality: Safe, Editable: true, Description: "The most tracks the serve sweep keeps waiting in word-timing re-check at once; each sweep cycle tops that count back up to this many (scan reconcile-word-sync --limit queues independently of this cap). Default 100, range 1-1000. A file value outside the range resets to the default; an out-of-range environment variable is ignored instead, so the file (or default) value stands. Each re-checked track costs at least one provider request at the worker's pace."},
