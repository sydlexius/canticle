...
var fields = []FieldSpec{
	...
	// [timing_validation]
	{Path: "timing_validation.enabled", Section: "timing_validation", Type: TypeBool, EnvVars: []string{"MXLRC_TIMING_VALIDATION_ENABLED"}, Criticality: Safe, Editable: true, Description: "Master switch for the serve-mode timing sweep, which re-judges .lrc files already on disk against their companion audio's exact duration. NOT YET ACTIVE: nothing reads this yet, so turning it on has no runtime effect; the sweep that consumes it lands separately (#443). The revalidate CLI command runs regardless. Off by default."},
	{Path: "timing_validation.revalidate_existing", Section: "timing_validation", Type: TypeBool, EnvVars: []string{"MXLRC_TIMING_VALIDATION_REVALIDATE_EXISTING"}, Criticality: Caution, Editable: true, Description: "Allow the sweep to drain the backlog of .lrc files written before the accept-time timing guard existed. NOT YET ACTIVE: no runtime effect until the sweep lands (#443). Both this and timing_validation.enabled must be true for the sweep to run. Off by default."},
	{Path: "timing_validation.revalidate_batch", Section: "timing_validation", Type: TypeInt, EnvVars: []string{"MXLRC_TIMING_VALIDATION_REVALIDATE_BATCH"}, Criticality: Safe, Editable: true, Description: "How many sidecars one sweep cycle judges. NOT YET ACTIVE: no runtime effect until the sweep lands (#443). Default 100, kept modest so the backlog drains over many cycles rather than in one burst that wakes the library disks. Values below 1 are reset to the default."},
	{Path: "timing_validation.on_mis_synced", Section: "timing_validation", Type: TypeString, EnvVars: []string{"MXLRC_TIMING_VALIDATION_ON_MIS_SYNCED"}, Criticality: Caution, Editable: true, Description: "What the sweep will do to a lyric that overran the audio but stayed under the categorical ratio. NOT YET ACTIVE: nothing reads this yet, so no file is moved or deleted whatever you pick (#443). One of: demote (keep the words as .txt, the default, since the words are content-correct), quarantine, purge (irreversible), off (evaluate and record only)."},
	{Path: "timing_validation.on_categorical", Section: "timing_validation", Type: TypeString, EnvVars: []string{"MXLRC_TIMING_VALIDATION_ON_CATEGORICAL"}, Criticality: Caution, Editable: true, Description: "What the sweep will do to a lyric whose last cue sits far past the audio end -- the wrong song's words entirely. NOT YET ACTIVE: nothing reads this yet, so no file is moved or deleted whatever you pick (#443). One of: quarantine (the default), purge (irreversible), off (evaluate and record only). There is no demote here: nothing is worth keeping as .txt."},
