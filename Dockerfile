...
# Raise/drop when the base index ships it by default.
# util-linux floor pinned to remediate the 2.42.1-r0 batch: 4 HIGH
# (CVE-2026-76642, CVE-2026-78408, CVE-2026-78409, CVE-2026-78410), a MEDIUM
# (CVE-2026-27456) and 3 LOW (CVE-2026-53612, CVE-2026-53613, CVE-2026-53614),
# all eight fixed in 2.42.3-r0 per the Alpine 3.24 secdb. It supplies
# libblkid/libmount, which is what the scanner names; pinned on `util-linux`
# (the ORIGIN package apk resolves the floor against), same as openssl/expat
# above. Arrives TRANSITIVELY, so `apk upgrade` alone leaves 2.42.1-r0 in place
# while the base digest predates the fix.
# Raise/drop when the base index ships it by default.
RUN apk add --no-cache bash ca-certificates "ffmpeg>=8.1.2-r0" "mbedtls3>=3.6.7-r0" "libssh>=0.12.2-r0" "openssl>=3.5.8-r0" "expat>=2.8.4-r0" "util-linux>=2.42.3-r0" su-exec tzdata && \
